...
| 区域 | 定位器 | 说明 |
|------|--------|------|
| 面包屑 | `getByRole('link', { name: '销售对账管理' })` | 当前页面面包屑 |
...
| 销售子单标签 | `getByRole('tab', { name: '销售子单' })` | 标签页 |
...
- 页面加载完成 → URL 为 `/orders/customer-bill`，表格可见，面包屑显示「销售对账管理」
- 搜索有效 → 输入关键词后表格数据更新
