...
| 区域 | 定位器 | 说明 |
|------|--------|------|
| 面包屑 | `getByRole('link', { name: '销售定价管理' })` | 当前页面面包屑 |
| id 输入框 | `getByRole('textbox', { name: 'id' })` | 文本输入 |
| 名称输入框 | `getByRole('textbox', { name: '名称' })` | 文本输入 |
| 合同下拉框 | `getByRole('combobox', { name: '合同' })` | 下拉选择 |
...
| 关联采购价格标签 | `getByRole('tab', { name: '关联采购价格' })` | 标签页 |
...
- 页面加载完成 → URL 为 `/orders/sale-price`，表格可见，面包屑显示「销售定价管理」
- 搜索有效 → 输入关键词后表格数据更新
