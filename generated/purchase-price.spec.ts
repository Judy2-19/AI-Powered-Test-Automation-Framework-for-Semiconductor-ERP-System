...
const PAGE_URL = '/orders/purchase-price'
...
test.describe('采购定价管理 P0', () => {
  ...
  test('页面加载完成，表格数据可见', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: '采购定价管理' })).toBeVisible()
  ...
  test('标签页切换：关联销售价格', async ({ page }) => {
    await expect(page.getByRole('tab', { name: '关联销售价格' })).toHaveClass(/is-active/)
  })
})
