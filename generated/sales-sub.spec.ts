...
test.describe('销售子单管理 P0', () => {
  ...
  test('页面加载完成，表格数据可见', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: '销售子单管理' })).toBeVisible()
    ...
    await expect(page.getByRole('button', { name: '当页导出' })).toBeVisible()
  ...
  test('费用确认操作：点击费用确认按钮', async ({ page }) => {
    ...
    const button = page.getByRole('button', { name: '费用确认' })
    await button.click()
  ...
  test('对账操作：点击对账按钮', async ({ page }) => {
    ...
    const button = page.getByRole('button', { name: '对账' })
    await button.click()
  ...
  test('开票操作：点击开票按钮', async ({ page }) => {
    ...
    const button = page.getByRole('button', { name: '开票' })
    await button.click()
  ...
  test('付款操作：点击付款按钮', async ({ page }) => {
    ...
    const button = page.getByRole('button', { name: '付款' })
    await button.click()
  ...
  test('同步金蝶操作：点击同步金蝶按钮', async ({ page }) => {
    const rows = page.locator('.el-table__row')
    if ((await rows.count()) > 0) {
      await rows.first().click()
      await page.waitForTimeout(1000)
    ...
    const button = page.getByRole('button', { name: '同步金蝶' })
    await button.click()
  ...
  test('金蝶红冲操作：点击金蝶红冲按钮', async ({ page }) => {
    const rows = page.locator('.el-table__row')
    if ((await rows.count()) > 0) {
      await rows.first().click()
      await page.waitForTimeout(1000)
    ...
    const button = page.getByRole('button', { name: '金蝶红冲' })
    await button.click()
  ...
  test('展开搜索条件：点击展开按钮显示更多搜索字段', async ({ page }) => {
    await page.getByRole('button', { name: '展开' }).click()
    await page.waitForTimeout(1000)
  })

  test('展开后搜索：使用展开后的搜索条件搜索', async ({ page }) => {
    await page.getByRole('button', { name: '展开' }).click()
    await page.waitForTimeout(1000)

    const searchFields = [
      page.getByRole('textbox', { name: '订单名称' }),
      page.getByRole('combobox', { name: '销售验收' }),
      page.getByRole('textbox', { name: '主单名称' }),
      page.getByRole('combobox', { name: '业务类型' })
    ]
  ...
  test('销售资金关联：点击销售资金标签的关联按钮', async ({ page }) => {
    const rows = page.locator('.el-table__row')
    if ((await rows.count()) > 0) {
      await rows.first().click()
      await page.waitForTimeout(1000)
    ...
    const fundTab = page.locator('.el-tabs__item').filter({ hasText: '销售资金' })
    await fundTab.first().click()
    await page.waitForTimeout(1000)

    const fundPane = page.locator('.el-tab-pane').filter({ hasText: '销售资金' })
