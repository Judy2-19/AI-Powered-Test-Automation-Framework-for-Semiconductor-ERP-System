...
const PAGE_URL = '/orders/customer-bill'
...
test.describe('销售对账管理 P0', () => {
  ...
  test('页面加载完成，表格数据可见', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: '销售对账管理' })).toBeVisible()

    await expect(page.getByRole('textbox', { name: 'id' })).toBeVisible()
    await expect(page.getByRole('textbox', { name: '对账单名称' })).toBeVisible()
    await expect(page.getByRole('textbox', { name: '对账单编号' })).toBeVisible()

    await expect(page.getByRole('button', { name: '搜索' })).toBeVisible()
    await expect(page.getByRole('button', { name: '重置' })).toBeVisible()
    await expect(page.getByRole('button', { name: '新增' })).toBeVisible()
    await expect(page.getByRole('button', { name: '开发票' })).toBeVisible()
    await expect(page.getByRole('button', { name: '导出' })).toBeVisible()

    await expect(page.locator('.el-table').first()).toBeVisible()
  })

  test('搜索功能：输入对账单名称搜索', async ({ page }) => {
    const searchInput = page.getByRole('textbox', { name: '对账单名称' })
  ...
  test('标签页切换：销售子单', async ({ page }) => {
    await expect(page.getByRole('tab', { name: '销售子单' })).toHaveClass(/is-active/)
  })
})
