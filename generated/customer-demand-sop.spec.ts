...
/**
 * 客户SOP管理 P0 测试
 * 页面地图：tests/context/pages/customer-demand-sop.md
 ...
 *
 * 定位器说明：
 * - 标签页：getByRole('tab', { name: '已接需求' / '需求大厅' })
 * - 搜索表单：getByLabel('id') / getByLabel('样品名称') / getByLabel('SOP模板')
 * - 按钮：getByRole('button', { name: '搜索' / '重置' / '新增' / '展开' })
...
const PAGE_URL = '/orders/customer-demand-sop'
...
test.describe('客户SOP管理 P0', () => {
  ...
  test('页面加载完成，表格数据可见', async ({ page }) => {
    ...
    // 面包屑显示正确
    await expect(page.getByRole('link', { name: '客户SOP管理' })).toBeVisible()

    // 标签页可见
    await expect(page.locator('.el-tabs__item').filter({ hasText: '已接需求' })).toBeVisible()
    await expect(page.locator('.el-tabs__item').filter({ hasText: '需求大厅' })).toBeVisible()
    ...
    await expect(page.locator('.el-form-item').filter({ hasText: 'SOP模板' }).locator('.el-select').first()).toBeVisible()
  ...
  test('标签页切换：已接需求/需求大厅切换', async ({ page }) => {
    // 当前在已接需求标签
    await expect(page.locator('.el-tabs__item').filter({ hasText: '已接需求' })).toHaveClass(/is-active/)

    // 切换到需求大厅
    await page.locator('.el-tabs__item').filter({ hasText: '需求大厅' }).click()
    await expect(page.locator('.el-tabs__item').filter({ hasText: '需求大厅' })).toHaveClass(/is-active/)

    // 切换回已接需求
    await page.locator('.el-tabs__item').filter({ hasText: '已接需求' }).click()
    await expect(page.locator('.el-tabs__item').filter({ hasText: '已接需求' })).toHaveClass(/is-active/)
  })
  ...
  test('更多操作：点击更多，执行SOP重填/推送/建订单/复制/删除', async ({ page }) => {
    ...
    await expect(page.getByRole('menuitem', { name: 'SOP重填' })).toBeVisible({ timeout: 5000 })
    await expect(page.getByRole('menuitem', { name: '推送' })).toBeVisible()
    await expect(page.getByRole('menuitem', { name: '建订单' })).toBeVisible()
  ...
  test('表格排序：点击SOP模板列排序箭头', async ({ page }) => {
    const headers = page.locator('.el-table__header').locator('th')
    const sopTemplateHeader = headers.nth(4)
    await expect(sopTemplateHeader).toBeVisible()

    await sopTemplateHeader.click()
    await page.waitForTimeout(1000)
  ...
  test('表格排序：点击是否智能体列排序箭头', async ({ page }) => {
    const headers = page.locator('.el-table__header').locator('th')
    const aiHeader = headers.nth(5)
